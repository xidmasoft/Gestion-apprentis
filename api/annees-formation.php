    ...
    if (!empty($data->libelle)) {
        try {
            ...
            if ($stmt->execute()) {
                ...
                echo json_encode(["message" => "Année de formation créée."]);
            }
        } catch (PDOException $e) {
            http_response_code(503);
            echo json_encode(["message" => "Impossible de créer l'année."]);
        }
    } else {
        http_response_code(400);
        echo json_encode(["message" => "Libellé obligatoire."]);
    }
}

function update($db) {
    $data = json_decode(file_get_contents("php://input"));
    if (!empty($data->id) && !empty($data->libelle)) {
        try {
            ...
            if ($stmt->execute()) {
                http_response_code(200);
                echo json_encode(["message" => "Année mise à jour."]);
            }
        } catch (PDOException $e) {
            http_response_code(503);
            echo json_encode(["message" => "Impossible de mettre à jour l'année."]);
        }
    } else {
        http_response_code(400);
        echo json_encode(["message" => "ID et Libellé obligatoires."]);
    }
}
...
function delete($db) {
    ...
    if (!empty($data->id)) {
        try {
            $query = "DELETE FROM annees_formation WHERE id=:id";
            $stmt = $db->prepare($query);
            $id = strip_tags($data->id);
            $stmt->bindParam(':id', $id);
            if ($stmt->execute()) {
                http_response_code(200);
                echo json_encode(["message" => "Année de formation supprimée."]);
            }
        } catch (PDOException $e) {
            http_response_code(503);
            if ($e->getCode() == 23000) {
                // Contrainte de clé étrangère
                $query = "UPDATE annees_formation SET actif=0 WHERE id=:id";
                $stmt = $db->prepare($query);
                $id = strip_tags($data->id);
                $stmt->bindParam(':id', $id);
                $stmt->execute();
                echo json_encode(["message" => "Impossible de supprimer cette année car des apprenants y sont liés. Elle a été désactivée."]);
            } else {
